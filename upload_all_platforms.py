"""
Velocity Korean - Unified Social Media Upload Script
...
def get_latest_reel():
    ...
    return {
        ...
        "metadata": metadata,
        "category": metadata.get("category_english", "Korean Learning"),
...
def generate_caption(phrases, category, platform="facebook"):
    ...
    if platform == "facebook":
        ...
        caption_lines = [
            f"🇰🇷 Learn Korean with Velocity Korean! 🇰🇷",
            f"",
            f"📚 Category: {category}",
            f"",
            f"🎯 Master Korean one phrase at a time! Today's {category} lesson:",
            f""
        ]
        ...
        for i, phrase in enumerate(phrases[:5], 0):
            ...
            caption_lines.append(f"   📍 {phrase['korean']}")
            caption_lines.append(f"   🔊 [{phrase.get('romanization', '')}]")
        ...
        caption_lines.extend([
            ...
            f"🔔 Follow for daily Korean lessons!",
            f"",
...
def upload_to_all_platforms(video_path, caption, category):
    ...
    print("\n" + "="*80)
    print("🚀 VELOCITY KOREAN - MULTI-PLATFORM UPLOAD")
    print("="*80)
...
def main():
    ...
    print("\n" + "="*80)
    print("🇰🇷 VELOCITY KOREAN - AUTOMATED UPLOAD 🇰🇷")
    print("="*80)
